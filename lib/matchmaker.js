...
export default class Matchmaker {
  ...
  basenamePattern(path) {
    ...
    // that from the name. If it does NOT end with `_test` or `_spec` or
    // `-spec`, then add those items as optional suffixes for the pattern.
    if (basename.match("(_test|_spec|-spec)$")) {
      return basename.replace(new RegExp("(_test|_spec|-spec)$"), "")
    } else {
      return `${basename}?(_test|_spec|-spec)`
    }
  }
}
