...
class Matchmaker {
  ...
  async complementaryPath () {
    let _complementaryPath
    for (const projectPath of atom.project.getPaths()) {
      _complementaryPath = await this.findComplementaryPath(projectPath, this.path)
      if (_complementaryPath) return _complementaryPath
    }

    return _complementaryPath
  }
  ...
  //
  // Returns a Promise.
  findComplementaryPath (directoryToSearch, matcheePath, leadingDirectoriesToExclude = 1) {
    ...
    const promise = new Promise((resolve) => {
      ...
      const scanner = new PathScanner(directoryToSearch, {
    ...
    return promise.then((matches) => {
      if (matches.length > 0) {
        ...
        return this.findComplementaryPath(directoryToSearch, matcheePath, leadingDirectoriesToExclude + 1)
      } else {
        return null
      }
    })
